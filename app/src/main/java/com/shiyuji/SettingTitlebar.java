...
public class SettingTitlebar extends LinearLayout {
    ...
    public SettingTitlebar(Context context, AttributeSet attrs) {
        super(context, attrs);
        LayoutInflater.from(context).inflate(R.layout.settings_titlebar, this);     // 调用inflate()方法将布局实例化为View对象

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SettingTitlebar);    // 使用TypedArray获取自定义属性集合

        titlebarText = (TextView) findViewById(R.id.titlebarText);
        titlebarPrev = (LinearLayout) findViewById(R.id.titlebarPrev);
        text = typedArray.getString(R.styleable.SettingTitlebar_titlebarText);     // 获得属性titlebarText的值
